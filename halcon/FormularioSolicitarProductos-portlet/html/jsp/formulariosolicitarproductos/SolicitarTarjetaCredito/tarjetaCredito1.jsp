...
<form action="/solicitud/tarjeta-credito/2" method="post" name="<portlet:namespace/>fm1" id="<portlet:namespace/>fm1">
<input type="hidden"  id="defaultTypeSex" value="${tipoSexo}" />
  <input type="hidden" id="hiddenF1" name="hiddenF1" value="<portlet:namespace/>fm1" />	
 	 <!--content-medio-->
    <div id="content-medio" class="sol">
      ...
      <div class="sol-formulario">
        ...
        <div class="campos-oblig">
          ...
          <input type="text" class="valid['alphaNumeric'] textform1 wt224 float-left" value="<%=apePater%>" id="apePater" name="apePater" maxLength="50"/>
        </div>
		  <div class="form-error" id="msj_apePater">${MSJ_APELLIDO_PATERNO}</div>
        
        <div class="campos-oblig">
          <label>Apellido Materno</label>
          <input type="text" class="valid['optionalalphaNumericGroup2'] textform1 wt224 float-left" value="<%=apeMater%>" id="apeMater" name="apeMater" maxLength="50"/>
        </div>
        <div class="form-error" id="msj_apeMater">${MSJ_APELLIDO_MATERNO}</div>
        
        <div class="campos-oblig">
          <label>Primer Nombre</label>
          <input type="text" class="valid['alphaNumeric'] textform1 wt224 float-left" value="<%=priNombre%>" id="priNombre" name="priNombre" maxLength="50"/>
        </div>
		  <div class="form-error" id="msj_priNombre">${MSJ_PRIMER_NOMBRE}</div>

        <div class="campos-oblig">
          <label>Segundo Nombre (Opcional)</label>
         <input type="text" class="valid['optionalalphaNumeric'] textform1 wt224 float-left" value="<%=segNombre%>" id="segNombre" name="segNombre" maxLength="50"/>
        </div>
        ...
        <div class="form-error">Debes ingresar el Sexo
        </div>
        
          <c:set var="valorEstadoCivil" value="0"/>
          <c:choose> 
          	<c:when test="${estadoCivil==null}"><c:set var="valorEstadoCivil" value="0"/></c:when>
          	<c:otherwise><c:set var="valorEstadoCivil" value="${estadoCivil}"/></c:otherwise>
          </c:choose>          
        <div class="campos-oblig">
          <label>Estado Civil</label>
          <div id="cboestadocivil"  class="valid['rdropdown'] dropdown2 wt190 float-left bgfa cboestadocivil" >
			<input type="text" name="cboestadocivilHidden" id="cboestadocivilHidden" value="${valorEstadoCivil}"  class="textdropdown" style="opacity: 0; z-index: -1; position: absolute;">
            <div>Seleccione</div>
          </div>
        </div>
        ...
        <c:set var="valorSituacionLaboral" value="0"/>
          <c:choose> 
          	<c:when test="${situacionLaboral==null}"><c:set var="valorSituacionLaboral" value="0"/></c:when>
          	<c:otherwise><c:set var="valorSituacionLaboral" value="${situacionLaboral}"/></c:otherwise>
          </c:choose>    
        <div class="campos-oblig">
          <label>Situaci&oacute;n Laboral</label>
          <div id="cbosituacionlaboral" class="valid['rdropdown'] dropdown2 wt190 float-left bgfa cbosituacionlaboral">
            <input type="text" name="cbosituacionlaboralHidden" id="cbosituacionlaboralHidden" value="${valorSituacionLaboral}"  class="textdropdown" style="opacity: 0; z-index: -1; position: absolute;">
            <div>Seleccione</div>
          </div>
        </div>
        ...
        <div class="campos-oblig oblig-ruc">
          ...
          <input type="text" class="valid['ruc'] textform1 wt224 float-left" id="ruc" name="ruc" maxLength="11" value="<%=ruc%>"/>
          <p class="con-decoration">Buscar el RUC <a target="_blank" href="http://www.sunat.gob.pe/cl-ti-itmrconsruc/jcrS00Alias" title="Buscar el RUC">aqu&iacute;</a></p>
        </div>
        <div class="form-error" id="msj_ruc" style="position:relative">
        <div style="top: 0px; left: 0px; position: absolute; margin: -15px 0px 0px;">${MSJ_RUC}</div></div>
        ...
        <div class="campos-oblig sin-margin">
          ...
          <div class="content-slider">
            <div id="vRenta" class="selector sl141">
              ...
              <img src="/halcon-theme/images/halcon/bkg_slider.jpg" class="sl-img" alt="Fondo Slider"/>
              <div class="knob"></div>
              <div class="sl-right"></div>
            </div>
            <input type="hidden" id="valorRenta" name="valorRenta" value="<%=valorRenta%>" class="input-sl" />
            <div class="contentText"> 
            	<span class="negra">S/.</span> <span class="valor-sl"></span> 
            </div>
          </div>
        </div>
        <div class="form-error">Debes ingresar la Renta Mensual
        </div>
        <div class="both"></div>
      </div>
...
<input type="hidden" id="urlPaso1_tj" value='<portlet:resourceURL id="paso1_tj"/>'/>
<input type="hidden" id="checkCodProducto" name="checkCodProducto" value="${checkCodProducto}" />
<input type="hidden" id="RANGO_PLAZO_TARJETA_CREDITO" name="RANGO_PLAZO_TARJETA_CREDITO" value='${RANGO_SLIDER_PERFILADOR_TARJETA}' />
...
<input type="hidden" id="estadoPaso1Credito" name="estadoPaso1Credito" value="${requestScope.estadoPaso1Credito}" />
<input type="hidden" id="estadoPaso2Credito" name="estadoPaso2Credito" value="${requestScope.estadoPaso2Credito}" />