...
window.addEvent("domready", function(){
	...
	if($('hiddenF1')){
		closeloadform();
		redireccionarURL();
		var rangoPlazoCreditoVehicular = $('RANGO_PLAZO_CREDITO_VEHICULAR').value;
		
		/*Tipo Documento*/
		getdatacombo("/GestionParametros-portlet/js/resources/tipo_documento_solicitud_credito_vehicular.json", "tipDoc", "cboDocumento");
		loadOptionGroup();
		validarDocument();
		validarDocumento();
		...
		if(valorRentaHidden=='null'){
			...
			setSelector2(vRenta, rangoPlazoCreditoVehicular,"S/.",false,false,'M\u00e1s de ');
		}else{
			//selector('vRenta',1000,20000,50,valorRentaHidden);
			setSelector2(vRenta, rangoPlazoCreditoVehicular,"S/.",false,valorRentaHidden,'M\u00e1s de ');
		}
...
//		});
		...
		new moovalid(hfrm,{
			...
			onValid:function(){
				loadform();
				//e.preventDefault();
				var urlEnvio=$('urlPaso1_cv').value;		
				validarDate();
	...
	if($('hiddenF2')){	
		closeloadform();
		redireccionarURL();
		//Generamos evento por cada selecciona de check de productos complementarios
		var productos= $('contentProductosComplementarios').getElements('input[type=checkbox]');
		var codProductoComple = $('checkCodProducto').get('value');
		if(codProductoComple != ""){
			codProductoComple = codProductoComple.split(",");
		...
		new moovalid(hfrm,{
			...
			onValid:function(){
				loadform();	
				var urlEnvio=$('urlPaso2_cv').value;
				//Obtenemos el codigo del check que esta seleccionado
				var checkCodProd = '';
				var productos= $('contentProductosComplementarios').getElements('input[type=checkbox]:checked');
	        	for ( var i = 0; i < productos.length; i++) {
	...
	if($('hiddenF3')){
		...
		new moovalid(hfrm,{
			...
			onValid:function(){
				loadform();	
				var urlEnvio=$('urlPaso3_cv').value;
				//Obtenemos el codigo del check que esta seleccionado
				var checkCodProd = '';
				var productos= $('contentProductosComplementarios').getElements('input[type=checkbox]:checked');
	        	for ( var i = 0; i < productos.length; i++) {
...
function loadOptionGroup(){
	...
	if(tipDoc=='1'){
		...
		getdatacombo("/GestionParametros-portlet/js/resources/tipo_documento_solicitud_credito_vehicular.json", "tipDoc", "cboDocumento",tipDoc);
	}
}
...
function redireccionarURL(){
	...
	if(paso>4){
		...
		var estado1=$('estadoPaso1Vehicular').value;
		var estado2=$('estadoPaso2Vehicular').value;
