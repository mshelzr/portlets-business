...
window.addEvent("domready", function(){
	...
	if($('hiddenF1')){
		closeloadform();
		redireccionarURL();
		var rangoPlazoTarjetaCredito = $("RANGO_PLAZO_TARJETA_CREDITO").value;
		
		/*Tipo Documento*/
		getdatacombo("/GestionParametros-portlet/js/resources/tipo_documento_solicitud_tarjeta_credito.json", "tipDoc", "cboDocumento");
		loadOptionGroup();
		validarDocument();
		validarDocumento();
		...
		if(valorRentaHidden=='null'){
			...
			setSelector2(vRenta, rangoPlazoTarjetaCredito,"S/.",false,false,'M\u00e1s de ');
		}else{
			//selector('vRenta',1000,20000,50,valorRentaHidden);
			setSelector2(vRenta, rangoPlazoTarjetaCredito,"S/.",false,valorRentaHidden,'M\u00e1s de ');
		}
		...
		new moovalid(hfrm,{
			...
			onValid:function(){
				loadform();
				//e.preventDefault();
				var urlEnvio=$('urlPaso1_tj').value;		
				validarDate();
	...
	if($('hiddenF2')){		
		...
		new moovalid(hfrm,{
			...
			onValid:function(){
				loadform();	
				var urlEnvio=$('urlPaso2_tj').value;
				//Obtenemos el codigo del check que esta seleccionado
				var checkCodProd = '';
				var productos= $('contentProductosComplementarios').getElements('input[type=checkbox]:checked');
	        	for ( var i = 0; i < productos.length; i++) {
	...
	/********************** Formulario 3 ************************************/
		...
		new moovalid(hfrm,{
			...
			onValid:function(){
				loadform();	
				var urlEnvio=$('urlPaso3_tj').value;
				//Obtenemos el codigo del check que esta seleccionado
				var checkCodProd = '';
				var productos= $('contentProductosComplementarios').getElements('input[type=checkbox]:checked');
	        	for ( var i = 0; i < productos.length; i++) {
...
function loadOptionGroup(){
	...
	if(tipDoc=='1'){
		getdatacombo("/GestionParametros-portlet/js/resources/tipo_documento_solicitud_tarjeta_credito.json", "tipDoc", "cboDocumento",false,false,false );	
	}else{
		getdatacombo("/GestionParametros-portlet/js/resources/tipo_documento_solicitud_tarjeta_credito.json", "tipDoc", "cboDocumento",tipDoc,false,false);
	}
}
...
function redireccionarURL(){
	...
	if(paso>4){
		...
		var estado1=$('estadoPaso1Credito').value;
		var estado2=$('estadoPaso2Credito').value;
