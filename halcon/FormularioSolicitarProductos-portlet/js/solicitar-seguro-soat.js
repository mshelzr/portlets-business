...
window.addEvent("domready", function(){
	...
	redireccionarURL();
	/*Tipo Documento*/
	getdatacombo("/GestionParametros-portlet/js/resources/tipo_documento_solicitud_seguro_soat.json", "tipDoc", "cboDocumento");
	loadOptionGroup();
	validarDocument();
	validarDocumento();
...
//			});
		...
		new moovalid(hfrm,{
			...
			onValid:function(){
				loadform();
				var urlEnvio=$('urlPaso1_ss').value;		
				validarDate();
	...
	if($('hiddenF2')){	
		...
		new moovalid(hfrm,{
			...
			onValid:function(){
				loadform();	
				var urlEnvio=$('urlPaso2_ss').value;
				//Obtenemos el codigo del check que esta seleccionado
				var checkCodProd = '';
				var productos= $('contentProductosComplementarios').getElements('input[type=checkbox]:checked');
	        	for ( var i = 0; i < productos.length; i++) {
				...
				var jsonRequest = new Request.JSON({
					...
					data : {
						...
						cbodistrito: $('cbodistritoHidden').value,
						cbodistritoTexto: $("cbodistrito").getElement("div").get('html'),
	...
	/********************** Formulario 3 ************************************/
		...
		new moovalid(hfrm,{
			...
			onValid:function(){
				loadform();	
				var urlEnvio=$('urlPaso3_ss').value;
				//Obtenemos el codigo del check que esta seleccionado
				var checkCodProd = '';
				var productos= $('contentProductosComplementarios').getElements('input[type=checkbox]:checked');
	        	for ( var i = 0; i < productos.length; i++) {
...
function cargarEventoClickProvincia(cbo) {
	cbo.addEvent('xChange', function(item) {
		var valor = item.get('valor');
		filtrarJson('get', "/GestionParametros-portlet/js/resources/distrito.json", "id_provincia", "cbodistritoHidden", "cbodistrito", valor);
	});
}
...
function loadOptionGroup(){
	...
	if(tipDoc=='1'){
		getdatacombo("/GestionParametros-portlet/js/resources/tipo_documento_solicitud_seguro_soat.json", "tipDoc", "cboDocumento",false,false,false );	
	}else{
		getdatacombo("/GestionParametros-portlet/js/resources/tipo_documento_solicitud_seguro_soat.json", "tipDoc", "cboDocumento",tipDoc,false,false);
	}
}
...
function redireccionarURL(){
	...
	if(paso>4){
		...
		var estado1=$('estadoPaso1Soat').value;
		var estado2=$('estadoPaso2soat').value;
