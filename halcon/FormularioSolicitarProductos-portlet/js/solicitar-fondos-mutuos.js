...
window.addEvent("domready", function(){
	...
	if($('hiddenF1')){		
		...
		redireccionarURL();
		/*Tipo Documento*/
		getdatacombo("/GestionParametros-portlet/js/resources/tipo_documento_solicitud_fondos_mutuos.json", "tipDoc", "cboDocumento");
		loadOptionGroup();
		validarDocument();
		validarDocumento();
		...
		new moovalid(hfrm,{
				...
				onValid:function(){
					loadform();
					var urlEnvio=$('urlPaso1_fm').value;		
					validarDate();
	...
	productos.each(function(el){
		...
		new moovalid(hfrm,{
			...
			onValid:function(){
				loadform();	
				var urlEnvio=$('urlPaso2_fm').value;
				//Obtenemos el codigo del check que esta seleccionado
				var checkCodProd = '';
				var productos= $('contentProductosComplementarios').getElements('input[type=checkbox]:checked');
	        	for ( var i = 0; i < productos.length; i++) {
	...
	/********************** Formulario 3 ************************************/
		...
		new moovalid(hfrm,{
			...
			onValid:function(){
				loadform();	
				var urlEnvio=$('urlPaso3_fm').value;
				//Obtenemos el codigo del check que esta seleccionado
				var checkCodProd = '';
				var productos= $('contentProductosComplementarios').getElements('input[type=checkbox]:checked');
	        	for ( var i = 0; i < productos.length; i++) {
...
function loadOptionGroup(){
	...
	if(tipDoc=='1'){
		getdatacombo("/GestionParametros-portlet/js/resources/tipo_documento_solicitud_fondos_mutuos.json", "tipDoc", "cboDocumento",false,false,false );	
	}else{
		getdatacombo("/GestionParametros-portlet/js/resources/tipo_documento_solicitud_fondos_mutuos.json", "tipDoc", "cboDocumento",tipDoc,false,false);
	}
}
...
function redireccionarURL(){
	...
	if(paso>4){
		...
		var estado1=$('estadoPaso1Mutuos').value;
		var estado2=$('estadoPaso2Mutuos').value;
