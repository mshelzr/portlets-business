...
function cargarButons() {
	...
	btnGrabar.addEvent('click', function(e) {
		...
		formSociosRemesa.addEvent("submit", function(event) {
			validarDocumentoIdentidad(event, formSociosRemesa, formSociosRemesa.get("action"), /^(xls|xlsx)$/, limite, "upload_target2", "archivo_masivo", function(resp) {
				if (resp.i == undefined) {
...
function iframeCreate(form, identificador, callback) {
	...
	i.addEvent('load', function() {
		// console.log(window.parent.frames[identificador].document.getElementById("jsonRespuesta"));
		var ret = window.parent.frames[identificador].document.getElementById("jsonRespuesta").innerHTML;
		// console.log("ret:"+ret);
		// window.parent.hola();
		// aca
		// se
