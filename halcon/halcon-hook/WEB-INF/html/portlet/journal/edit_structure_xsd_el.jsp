					...
					<c:otherwise>
						<aui:column>
							<select id="<portlet:namespace />structure_el<%= count.getValue() %>_type" tabindex="<%= tabIndex.getValue() %>">
								...
								<option <%= elType.equals("link_to_product") ? "selected" : "" %> value="link_to_product"><liferay-ui:message key="Producto Complementario" /></option>
								<option <%= elType.equals("link_to_solicitud") ? "selected" : "" %> value="link_to_solicitud"><liferay-ui:message key="Solicitud de Producto" /></option>
							</select>
						</aui:column>
