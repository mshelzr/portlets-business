...
by neil  14/01/2013 last version
*/
var SlimPicker = new Class({
    ...
    options: {
        ...
        calendarType: 'fullCalendar', //fullCalendar y smallCalendar
    ...
    setCurrentDate: function () {
        ...
        if (inputValue != '' && inputValue != 'dd/mm/aaaa' && inputValue != 'mm/aaaa') {
    ...
    show: function () {
        ...
        if (this.options.calendarType != 'fullCalendar') this.setInput();
    ...
    useSelection: function (el) {
        ...
        selectDate = this.formatValue(dateArray[0], dateArray[1], dateArray[2]);
        this.input.value = selectDate;
        this.fireEvent('onChangeDate');
        this.close();
    },
    ...
    create: function () {
        ...
        if (this.options.calendarType != 'fullCalendar') this.calendar.addClass("sp_calsmall");
    ...
    draw: function () {
        ...
        if (this.options.calendarType == 'fullCalendar') {
            str += '<tbody>';
            ...
            var row = 0,
        dayMonthNext = 0,
        dayMonthPrevious = this.lastDayMonthPrevious - (currentDay * -1),
        currentMonth = this.currentMonth.toInt() + 1;
            //currentYear = this.calendarYear.toInt() + 1;
            ...
            while (row < 6) {
                ...
                for (i = 0; i < 7; i++) {
                    if ((currentDay <= this.lastDayMonth) && (currentDay > 0) && (row <= 6)) {
                        ...
                        str += '<td><a href="#' + this.currentYear + '|' + currentMonth + '|' + currentDay + '" class="' + this.options.dayClass;
    ...
    addCalendarEvents: function () {
        ...
        if (this.options.showYear) {
            ...
            this.yearUp = this.calendar.getElement('div.spup');
            this.yearDown = this.calendar.getElement('div.spdown');
        ...
        if (this.options.showMonth) {
            this.monthNext = this.calendar.getElement('div.spnext');
            this.monthPrevious = this.calendar.getElement('div.spprevious');
    ...
    position: function () {
        ...
        var inputheight = this.input.getSize().y.toInt() - 1;
        this.container.setStyles({
            height: coords.height,
            width: coords.width,
            left: coords.left - 35,
            top: (this.options.calendarType == 'fullCalendar') ? coords.top : coords.top + inputheight
        });
    ...
    changedMonthPrevious: function () {
        if (this.currentMonth <= 11 && this.currentMonth > 0) {
            this.currentMonth = this.currentMonth.toInt() - 1;
            this.selectChanged();
            if (this.options.calendarType != 'fullCalendar') this.setInput();
        }
    },
    ...
    changedYearUp: function () {
        this.currentYear = this.currentYear.toInt() + 1;
    ...
    setInput: function () {
        if (this.options.calendarType != 'fullCalendar') this.setMonthYear
        var currentMonthNumeric = this.currentMonth + 1;
        currentMonthNumeric = ((currentMonthNumeric < 10) ? "0" + currentMonthNumeric : currentMonthNumeric);
        this.input.set("value", currentMonthNumeric + '/' + this.currentYear);
    },
    ...
    setMonthYear: function () {
        ...
        if (text != "mm/aaaa") {
            var sDay = text.split("/")
            this.currentYear = sDay[1].toInt();
            this.currentMonth = sDay[0].toInt() - 1;
        }
    },
