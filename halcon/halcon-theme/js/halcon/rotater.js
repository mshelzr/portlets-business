...
MGFX.Rotater = new Class({
	...
	createFx: function(){
		...
		var vTab = this.options.tabScroll;
		this.slides.each(function(slide){
			slide.setStyle('z-index','1');
			if(vTab != false) var myScrollable = new Scrollable(slide);
		}.bind(this));
	}.protect(),
	...
	showSlide: function(slideIndex){
		...
		this.slides.each(function(slide, index){
			if(index == slideIndex && index != curSlide){ //show
				if(this.options.tabScroll) scrollbar[index].setStyle('z-index','79');
				action[index.toString()] = {'z-index' : '79'};
			} else {
				if(this.options.tabScroll) scrollbar[index].setStyles({'z-index':'1','visibility':'hidden'});
				action[index.toString()] = {'z-index':'1'};
			}
