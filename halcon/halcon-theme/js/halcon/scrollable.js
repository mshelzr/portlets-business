...
var Scrollable = new Class({
	...
	initialize: function(element, options) {
		...
		if (typeOf(element) == 'elements') {
			...
			this.container = new Element('div',{'class': this.options.className, html: '<div class="knob"></div>'});
			this.container.inject(document.body, 'bottom');
			this.slider = new xSlider(this.container, this.container.getElement('div'), {
				mode: 'vertical',
			...
			this.knobtop = new Element('span',{"class":"top"});
			this.knobbody = new Element('span',{"class":"body"});
			this.knobbottom = new Element('span',{"class":"bottom"});
			this.knob.adopt(this.knobtop,this.knobbody,this.knobbottom);
			this.reposition();
		...
		if (this.options.proportional === true) {
			if (isNaN(this.options.proportionalMinHeight) || this.options.proportionalMinHeight <= 0) {
				...
				if(this.element.scrollHeight > this.element.offsetHeight ){
					...
					this.knobbody.setStyle('height', Math.max(knobHeight, minHeight) -14);
				}else{
					this.knob.setStyle('opacity','0');
				}
			}
		}

		//this.slider.set(Math.round((this.element.scrollTop / (this.element.scrollHeight - this.element.offsetHeight)) * 100));
		var iSteps = Math.round((this.element.scrollTop / (this.element.scrollHeight - this.element.offsetHeight)) * 100);
		if(isNaN(iSteps)) iSteps = 0;
		this.slider.set(iSteps);
