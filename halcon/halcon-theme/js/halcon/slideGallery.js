...
var slideGallery = new Class({
	...
	createPaging: function() {
		...
		//else {
		if(pagingHold == null) this.paging.inject(this.gallery).addClass("paging");
			//}
//		
...
var fadeGallery = new Class({
	...
	play: function(animate) {
		...
		if(this.options.mode == "line") this.sidesChecking();
		if(animate) {
			this.fxFade[this.previous].start(0).fireEvent('initialize',[this.items.setStyle('z-index','10'),this.items[this.current].setStyle('z-index','11')]);
