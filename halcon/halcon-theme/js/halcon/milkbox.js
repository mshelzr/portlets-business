...
this.Milkbox = new Class({
	...
	loadFile:function(fileObj,preloads){
		...
		} else if (this.checkFileType(fileObj,'jsp')){
			this.loadHtml(fileObj);
		} else if (this.checkFileType(fileObj,'html')){
			this.loadHtml(fileObj);
		} else if(this.checkFileType(fileObj,'')){//
			this.loadHtml(fileObj);
		} else {//filetype:image
	...
	preloadFiles:function(preloads){
		preloads.each(function(fileObj,index){
			if(!this.checkFileType(fileObj,"swf") &&  !this.checkFileType(fileObj,"html") && !this.checkFileType(fileObj,"jsp")){
	...
	checkFileType:function(file,type){
		...
		if(!test && (type=="html" || type=="jsp" || type==""/**/)){
...
//bottom: 0;
		...
		this.controls.adopt(this.close);
	},
	...
	show_file:function(file,caption,index,length){
		...
		$$(this.bottom,this.controls).setStyle('height',Math.max(this.caption.getDimensions().height,this.controls.getComputedSize().totalHeight));
		this.controls.setStyle('left',target_size.total_w.toInt() - this.controls.getStyle('width').toInt())
	...
	appear:function(){
		if(!this.options.centered){
			...
			this.mainbox.setStyle('top',this.options.marginTop+50);
		}
	...
	set_mode:function(gallery_type){
		...
		switch(gallery_type){
			...
			case 'standard':
				...
				//if(this.playpause){this.playpause.setStyle('display','none');}
...
var MilkboxGallery = new Class({
	...
	check_extension:function(string){
		...
		regular_file = splitted.test(/\.(gif|jpg|jpeg|png|swf|html|jsp)$/i);
